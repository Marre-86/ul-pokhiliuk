...
Реализован также ещё один эндпойнт API:
- **GET /api/notifications** - возвращает список всех оповещений указанного пользователя.
...
- Статический анализ кода [PHPSTan](https://phpstan.org/) проверяется командой `make phpstan`.
...
На момент сдачи приложения линтер не находит погрешностей, PHPSTan проходит проверку с уровнем 5, покрытие тестами составляет 74%.
...
- **redis**: Redis 8.6
- **queue-worker-high**: Воркер RabbitMQ для очереди задач высокого приоритета
- **queue-worker-low**: Воркер RabbitMQ для очереди задач низкого приоритета
